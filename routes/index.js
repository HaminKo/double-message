...
//       res.status(500).send(err)
//     } else {
//       Contact.findOne({email: req.body}, function(err, contact) {
//         if (err) {
//           res.status(500).send(err)
//         } else {
//           var newMessage = new Message ({
        ...
        if (err) {
          ...
          User.findById(message.user, function(err, user) {
            if (err) {
              ...
              client.sendMessage({
                to: '+1' + contact.phone, // Any number Twilio can deliver to
                from: '+1' + user.phone, // A number you bought from Twilio and can use for outbound communication
